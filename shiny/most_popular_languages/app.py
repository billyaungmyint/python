...
languages = pd.read_csv(f"D:\\Google Drive\\python\\shiny\\most_popular_languages\\Most Popular Programming Languages from 2004 to 2022.csv")
...
app_ui = ui.page_fluid(
    ...
    ui.layout_sidebar(
        ui.panel_sidebar(
            ui.input_selectize(id='language' , label = 'Languages' , choices=languages_names_dict ,
                            selected='Python' , multiple=True),
...
def server(input, output, session):
    ...
    def lang_filt():
        ...
        lang_filt = languages_long.loc[(languages_long['language'].isin(list(input.language()))) &
                                       (languages_long['datetime'] >= date_selected_start) * (languages_long['datetime'] <= date_selected_end)].reset_index(drop=True)
        return lang_filt
    
    @output
    @render.plot
    def plotTimeSeries():
        g = ggplot(lang_filt()) + aes(x = 'datetime' , y = 'popularity' , color ='language') + geom_line() + theme(axis_text_x=element_text(rotation=90,hjust=1)) + labs(x='Date' , y='Popularity over time')
        return g
